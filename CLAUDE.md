...
- Pré-game (inline em `App.tsx`, ainda estático — ver Sub-fase 8b pendente);
...
Fase 1 (Riot Sync), o refinamento visual do desktop, Fase 2 (Player Intelligence), Fase 3 (Draft Intelligence), Fase 4 (Post-Game Coach), Fase 5 (Growth Journey), a conexão do desktop às rotas reais, a Fase 6 inteira (Configurações + tema com skins + "quantas partidas analisar" + ordem de pick automática via LCU), a Fase 7 (auditoria e documentação dos algoritmos de scoring) e a Sub-fase 8a (motor de build + seletor de time inimigo) estão completas em `main`. Próximo:

1. **Sub-fase 8b (polimento visual do desktop)** — próxima da fila, plano já aprovado: componente de loading compartilhado, Pré-game enriquecido (ícones/splash do campeão confirmado + inimigos + resumo de inclinação de dano via `summarizeEnemyDamageLean`, já exportado pela 8a), ícones de campeão na lista do Pós-game (usando `fetchAllChampions`/`fetchChampionClassProfiles`, já buscados pela 8a — fecha o item de catálogo de campeões abaixo de quebra), ícones/cor de tendência em Evolução, pequenos ajustes de CSS.
