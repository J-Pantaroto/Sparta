...
Fase 1 (Riot Sync), o refinamento visual do desktop, Fase 2 (Player Intelligence), Fase 3 (Draft Intelligence), Fase 4 (Post-Game Coach), Fase 5 (Growth Journey), a conexão do desktop às rotas reais, a Fase 6 inteira (Configurações + tema com skins + "quantas partidas analisar" + ordem de pick automática via LCU), a Fase 7 (auditoria e documentação dos algoritmos de scoring), a Fase 8 inteira (Sub-fase 8a: motor de build + seletor de time inimigo; Sub-fase 8b: polimento visual do desktop, validada com a conta real Zekerus#117) e a Fase 9 inteira (Sub-fase 9a: `ScoreBadge`/`StatBar`/`SignalChip`, Dashboard, Champion Select; Sub-fase 9b: Perfil, Pós-game, Evolução; mais três bugs reais corrigidos ao longo do caminho — card de skin sobreposto, cor errada do número do `ScoreBadge`, e o preload nunca carregando de verdade em nenhuma sessão anterior) estão completas em `main`. Próximo:

1. **Validar o comportamento downstream do bridge `window.sparta` agora que ele existe de verdade** (ver "Bug real corrigido: preload nunca carregava de verdade") — a existência do bridge foi confirmada via CDP contra o processo Electron real, mas nenhum dos 3 recursos que dependem dele foi exercitado de ponta a ponta nesta sessão: detecção automática de champion select/ordem de pick (precisa do cliente do League aberto) e download de skin pro disco (precisa clicar o botão "Baixar" de verdade e conferir o arquivo em `userData/skins`).
