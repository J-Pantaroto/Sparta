...
fato posterior (Match-V5, timeline e observações normalizadas). Sinais
comparativos carregam ID, status, evidência de cada lado, proveniência e
limitação de não causalidade. Histórico de loadout usa somente observações
anteriores ao snapshot. Revisão de patch importada depois dele é marcada como
contexto posterior.

Cobertura é calculada sobre oito disponibilidades independentes e não mede
confiança, qualidade ou probabilidade. Zero observado permanece valor;
ausência permanece indisponível. Matchup global e impacto global observado do
patch continuam explicitamente indisponíveis.
