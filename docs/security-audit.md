...
## Etapa 31D — acesso obrigatório

Bearer persistido saiu do `localStorage` e passou ao `safeStorage` do processo principal; logout e
mudança de email invalidam sessões por `sessionVersion`. Conta inativa não autentica. Cadastro e
reenvio não enumeram emails; tokens de confirmação nunca são persistidos em claro, são de uso único
e expiráveis. Toda rota pessoal exige email confirmado e vínculo Riot aceito. Produção permanece
fail-closed sem provider real e sem RSO aprovado. Detalhes em `docs/account-access-onboarding.md`.

## Etapa 31C — identidade e ownership

Autorização central por rota, fail-closed, ownership derivado do token e 404 contra enumeração.
Produção recusa legado e fecha laboratório/administração. RSO usa `state` one-time com hash/TTL;
code e tokens Riot não são persistidos, logados nem enviados ao renderer/replay. Detalhes:
