...
| `GET /me/player-profile`                                                    | OWN_RESOURCE   | `sub` → conta própria               | perfil agregado próprio; sem ID arbitrário     | consulta deriva integralmente da sessão             |
...
| `GET /players/:puuid/match-history`                                         | OWN_RESOURCE   | `sub` → PUUID próprio               | histórico filtrável/paginado próprio           | mesmo enriquecimento de `/me/player-profile` (Etapa 31H) |
...
| `GET /matches/:matchId/participants`                                        | OWN_RESOURCE   | partida participante + conta        | 10 participantes de partida própria            | 404 cruzado (Etapa 31H)                             |
